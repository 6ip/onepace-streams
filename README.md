...
`stream/`, `meta/` and `catalog/` are regenerated by GitHub Actions (streams hourly, metadata every 6 hours). Pull requests against those files will be overwritten on the next run — open an issue instead, or send a PR for the scripts.
