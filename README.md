...
`tracker.json` tracks the last-known input state per file so unchanged episodes are skipped.
...
### `report.py` — Commit messages

Turns the staged diff into the commit message the Action uses.
...
`stream/` and `meta/` are regenerated by a GitHub Action every 3 hours. Pull requests against those files will be overwritten on the next run — open an issue instead, or send a PR for the scripts.

## License

The MIT license covers `scr.py`, `epis.py` and the rest of the tooling here.

It does **not** cover the data. Episode titles, descriptions, release names and the spreadsheet they come from are the work of [the One Pace project](https://onepace.net/) and its contributors; this repository only indexes and reformats them so the addon can read them.
